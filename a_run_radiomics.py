...
def convert_to_hu(dicom_file):
    ...
    pixel_values = (pixel_values * slope) + bias
    return pixel_values
...
def get_cts(CT_files):
    ...
    for ct_file in CT_files:
        ...
        if ds.SOPClassUID == '1.2.840.10008.5.1.4.1.1.2':
            ...
            slices[ds.ImagePositionPatient[2]] = convert_to_hu(ds)
        else:
            pass
...
def mask_intensity(ct_array, ct_spacing, ds_pxl, dimameter = 50, treshold=0):
    mask = None
    z_min = ds_pxl[2]-int(dimameter/ct_spacing[2]/2)
    z_max = ds_pxl[2]+int(dimameter/ct_spacing[2]/2)
    # print x_min, x_max, y_min, y_max, z_min, z_max
    ...
    for z_slice in range(z_min,z_max+1,1):
        ...
        if r2 >= 0:
            ...
            mask_slice[circle] = 1
            # print ct_array.shape, mask.shape
            mask_slice[ct_array[z_slice,:,:]<treshold] = 0   

            mask_slice = scipy.ndimage.binary_fill_holes(mask_slice, structure=np.ones((5, 5)))
            mask_slice = scipy.ndimage.binary_fill_holes(mask_slice, structure=np.ones((3, 3)))
            mask_slice = scipy.ndimage.binary_fill_holes(mask_slice)
            # mask_slice = morphology.flood_fill(mask_slice, (0, 0), 1, tolerance=1)
            mask[z_slice,:,:] = mask_slice
            

        else:
            print 'Error: r2 is smaller than zero'
    # mask[ct_array < treshold] = 0
    return mask


def mask_intensity_cy(ct_array, ct_spacing, ds_pxl, dimameter = 50, height=50, treshold=0):
    mask = None
    mask_slice = None
    z_min = ds_pxl[2]-int(height/ct_spacing[2]/2)
    z_max = ds_pxl[2]+int(height/ct_spacing[2]/2)
...
def get_lc_metadata(label='all'):
    ...
    with open (LC_LABEL_LIST, 'r') as csvfile:
        ...
        for row in csvreader:
            # print '>>', row
            if '2004158_20160922_0' not in row[1] and '5561190_20191211_5' not in row[1] and '2004158_20160922_0' not in row[1]: 
                if  label == 'all':
                    ct_list.append(row[1])
                    lc_list.append(json.loads(row[14])) # i changed it but not sure why
                    for i in range(len(row)):
                        lc_metadata[header[i]].append(row[i])
                elif label in row[3]:
                    ct_list.append(row[1])
                    lc_list.append(json.loads(row[14]))
                    for i in range(len(row)):
                        lc_metadata[header[i]].append(row[i])
                else:
                    print 'label invalid: ', row[3], label
...
def process(case_ID, patient_dicom_path, lesion_center, contour, size, treshold,vdp_nlp, cnt):
    feature_vector = None
    features = None
    patient_images_paths = None
    ct_array = None 
    ct_x = None
    ct_y = None
    ct_z = None
    ...
    lesion_center [2] = ct_array.shape[0]-lesion_center[2]-1
    ...
    if contour == 'SP':
        ...
        mask = mask_intensity(ct_array, ct_spacing, lesion_center, dimameter = size, treshold=treshold)
    elif contour == 'CYIT':
        mask = mask_intensity_cy(ct_array, ct_spacing, lesion_center, dimameter = size[0], height=size[1], treshold=treshold)
    
    # print mask
    # 
    # mask[ct_array<80] = None
    # axs.hist(ct_array[mask==1].flatten(),bins=100)
    
    # Find contours:
    # mask = mask.filter(ImageFilter.FIND_EDGES)
    mask[mask==0] = None
    crop_size = 30
    print '___________>>', vdp_nlp
    if (vdp_nlp=="pain" or vdp_nlp=="no pain"):
        y_min_plot = lesion_center[1]-crop_size
        y_max_plot = lesion_center[1]+crop_size
        x_min_plot = lesion_center[0]-crop_size
        x_max_plot = lesion_center[0]+crop_size
        z_min_plot = int(lesion_center[2]-crop_size*ct_spacing[0]/ct_spacing[2])
        z_max_plot = int(lesion_center[2]+crop_size*ct_spacing[0]/ct_spacing[2])
        print('....',ct_array.shape)
        print(x_min_plot,x_max_plot,y_min_plot,y_max_plot,z_min_plot,z_max_plot)
        
        if x_min_plot < 0:
            x_max_plot -= x_min_plot
            x_min_plot = 0
        if x_max_plot > 512:
            x_min_plot -= (512-x_max_plot)
            x_max_plot = 512
        if y_min_plot < 0:
            y_max_plot -= y_min_plot
            y_min_plot = 0
        if y_max_plot > 512:
            y_min_plot -= (512-y_max_plot)
            y_max_plot = 512
        if z_min_plot < 0:
            z_max_plot -= z_min_plot
            z_min_plot = 0
        if z_max_plot > ct_array.shape[0]:
            z_min_plot -= (ct_array.shape[0]-z_max_plot)
            z_max_plot = ct_array.shape[0]
        print(x_min_plot,x_max_plot,y_min_plot,y_max_plot,z_min_plot,z_max_plot)
        #fig, axs = plt.subplots(1,1)
        #axs.set_title('%s_%s_x,y_%s_%s'%(cnt,size,crop_size,vdp_nlp))
        print 'size:   ', size
        if size == 50:
            axs.set_title('%s_%s_size_x,y_%s_%s'%(cnt,size,crop_size, vdp_nlp))
            axs.imshow(ct_array[lesion_center[2],y_min_plot:y_max_plot,x_min_plot:x_max_plot], cmap='gray', origin='lower',aspect = ct_spacing[1]/ct_spacing[0])
            plt.savefig('./supplementary/m_ROIs/%s_%s_size_x,y_%s_%s.png'%(cnt,size,crop_size,vdp_nlp))
            #axs.set_title('%s_%s_size_x,z_%s_%s'%(cnt,size,crop_size, vdp_nlp))
            #axs.imshow(ct_array[z_min_plot:z_max_plot,lesion_center[1],x_min_plot:x_max_plot], cmap='gray', origin='lower',aspect = ct_spacing[2]/ct_spacing[0])
            #plt.savefig('./supplementary/m_ROIs/%s_%s_size_x,z_%s_%s_p.png'%(cnt,size,crop_size,vdp_nlp))
            #axs.set_title('%s_%s_size_y,z_%s_%s'%(cnt,size,crop_size, vdp_nlp))
            #axs.imshow(ct_array[z_min_plot:z_max_plot,y_min_plot:y_max_plot,lesion_center[0]], cmap='gray', origin='lower',aspect = ct_spacing[2]/ct_spacing[1])
            #plt.savefig('./supplementary/m_ROIs/%s_%s_size_y,z_%s_%s.png'%(cnt,size,crop_size,vdp_nlp))
        
        axs.imshow(mask[ lesion_center[2], y_min_plot:y_max_plot,x_min_plot:x_max_plot], vmin=0, vmax=30/size, origin='lower', alpha=0.3,aspect = ct_spacing[1]/ct_spacing[0])
        #axs.imshow(mask[z_min_plot:z_max_plot, lesion_center[1],x_min_plot:x_max_plot], vmin=0, vmax=30/size, origin='lower', alpha=.4, aspect = ct_spacing[2]/ct_spacing[0])
        #axs.imshow(mask[z_min_plot:z_max_plot,y_min_plot:y_max_plot, lesion_center[0]], vmin=0, vmax=30/size, origin='lower', alpha=0.4,aspect = ct_spacing[2]/ct_spacing[1])
...
def get_radiomics(contour, size, treshold, label):
    if  'CY' in contour:
        if size[0] == size[1]:
    ...
    cts_path, lcs_list, lc_metadata = get_lc_metadata(label=label)
    # print lc_metadata
    print 'parsing: %s files'%len(cts_path)
    # print g
    ...
    medians = [0]
    t0 = datetime.now()
    point = 62 # 25, 58, 61, 62,
    features = process(case_ids[point], cts_path[point], 
        lcs_list[point],contour, size, 
        treshold,lc_metadata['vdp_nlp'][point],point)
    return None
    for i in range(len(case_ids)):
        ...
        if skip_processed and (label_name in already_processed_ids or '0_0_0' in label_name):
            print 'Processed'
        else:
            cnt_p += 1
            try:
                t1 = datetime.now()
                features = process(case_ids[i], cts_path[i], lcs_list[i],contour, size, treshold,lc_metadata['vdp_nlp'][i],i)
...
def main():
    ...
    # use 'MET' for met points, 'all' for all
    label = 'MET' # 'all', 'MET'
    n =3
    m = n+6
    c = 2
    ...
    # sizes = [[50,50]]
    contours = ['SP', 'CY','SPIT','CYIT'][c:c+1]
    # contours = ['CY']
    for size in sizes:
        for contour in contours:
            gc.collect()
            get_radiomics(contour, size, treshold, label)
